...
import db
from db import DB_PATH, KIND_LIVE, KIND_STATIC
from utils import (
...
async def cmd_admin(message: Message):
    ...
    await message.answer(
        ...
        "• «Автодобор сейчас» — добрать поток до 3 / предложить «сегодня»\n",
...
async def menu_cb(call: CallbackQuery, state: FSMContext):
    ...
    if call.data == "menu:book_live":
        ...
        start = next_working_day(today) if is_working_day(today) else next_working_day(today)
...
async def home_text(message: Message, state: FSMContext):
    await go_home(message, state)
...
# ===================== MAIN =====================
async def main():
    load_dotenv(dotenv_path=_project_root_env_path())

    bot_token = os.getenv("BOT_TOKEN", "")
    admin_id = get_admin_id()

    if not bot_token:
        raise RuntimeError("BOT_TOKEN is empty. Check .env")
    if not admin_id:
        raise RuntimeError("ADMIN_ID is empty. Check .env")
    ...
    # reply HOME
    dp.message.register(home_text, F.text == SETTINGS.BTN_HOME)
    ...
    # admin: manual add MUST be before adm:
    dp.callback_query.register(admin_add_start, F.data == "adm:add")
