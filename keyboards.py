...
def admin_time_approve_kb(bid: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="✅ Ок, принимаю", callback_data=f"adm:approve_yes:{bid}")],
        [InlineKeyboardButton(text="❌ Не подходит", callback_data=f"adm:approve_no:{bid}")],
    ])
...
def admin_queue_keyboard(day: date, is_today: bool, day_rows, in_service_rows, parts_rows) -> InlineKeyboardMarkup:
    ...
    for bid, seq, car, _issue, _phone, status, _uid, _kind, _eta, _mco, needs_ok in day_rows:
        ...
        # waiting: можно позвать и можно взять в работу (по вашему требованию)
        ...
        # arrived: взять в работу (обычное) + неявка
    ...
    # В работе сейчас — дублей тут нет, только “Готово/Ждёт запчасть” с описанием
    if in_service_rows:
