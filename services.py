...
from keyboards import arrived_kb, offer_today_kb, accept_kb, admin_time_approve_kb, reschedule_suggest_kb
...
async def client_arrived(bot: Bot, admin_id: int, bid: int, user_id: int) -> tuple[bool, str]:
    ...
    async with aiosqlite.connect(DB_PATH) as conn:
        ...
        if status not in (STATUS_CALLED, STATUS_ARRIVED):
            await conn.commit()
        ...
        cur = await conn.execute("""
            ...
            WHERE id=? AND user_id=? AND status IN ('called','arrived')
...
# -------- клиент прислал время -> ждём решение админа --------
...
async def admin_force_accept(bot: Bot, admin_id: int, bid: int) -> tuple[bool, str]:
    """
    Взять в работу даже если клиент не нажал “Я подъехал”.
    Разрешаем из waiting/called/arrived.
    """
    ts = db.now_iso()
    ...
    async with aiosqlite.connect(DB_PATH) as conn:
        ...
        cur = await conn.execute("""
            SELECT user_id
            FROM bookings
            WHERE id=? AND status IN ('waiting','called','arrived')
        """, (int(bid),))
        row = await cur.fetchone()
        if not row:
            await conn.commit()
            return False, "⛔ Можно взять в работу только если статус waiting/called/arrived."

        user_id = int(row[0] or 0)

        cur = await conn.execute("""
            UPDATE bookings
            SET status='in_service', updated_at=?
            WHERE id=? AND status IN ('waiting','called','arrived')
        """, (ts, int(bid)))
        await conn.commit()
